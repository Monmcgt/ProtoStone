...
/**
 * <a href="https://github.com/DeadlyMC/ProtoSky/blob/9b938afbddf9f5ad5db4099d6f5a43129ae34071/src/main/java/protosky/stuctures/PillarHelper.java">Source</a>
 */
public class PillarUtil {
    ...
    private static boolean spikeChunkIntersects(EndSpikeFeature.Spike circle, ProtoChunk rect) {
        ...
        if (circleDistance_x > (16 / 2 + circle.getRadius())) {
            return false;
        }
        if (circleDistance_y > (16 / 2 + circle.getRadius())) {
            return false;
        }

        if (circleDistance_x <= (16 / 2)) {
            return true;
        }
        if (circleDistance_y <= (16 / 2)) {
            return true;
        }
    ...
    public static void generate(StructureWorldAccess world, ProtoChunk chunk) {
        ...
        if (chunkX >= -6 && chunkX <= 6 && chunkZ >= -6 && chunkZ <= 6) {
            for (EndSpikeFeature.Spike spike : EndSpikeFeature.getSpikes(world)) {
                if (spikeChunkIntersects(spike, chunk)) generateSpike(world, spike, chunk);
            }
        }
    }
    ...
    public static void generateSpike(ServerWorldAccess world, EndSpikeFeature.Spike spike, ProtoChunk chunk) {
        ...
        for (BlockPos blockPos : BlockPos.iterate(
            ...
            if (blockPos.getSquaredDistance(spike.getCenterX(), blockPos.getY(), spike.getCenterZ()) <= (double) (i * i + 1) && blockPos.getY() < spike.getHeight()) {
                StructureUtil.setBlockInChunk(chunk, blockPos, Blocks.OBSIDIAN.getDefaultState());
                //.info("trying to generate");
            } else if (blockPos.getY() > 65) {
                StructureUtil.setBlockInChunk(chunk, blockPos, Blocks.AIR.getDefaultState());
            }
                ...
                for (int n = -2; n <= 2; ++n) {
                    for (int o = 0; o <= 3; ++o) {
                        ...
                        if (bl || bl2 || bl3) {
                            ...
                            StructureUtil.setBlockInChunk(chunk, mutable.set(spike.getCenterX() + m, spike.getHeight() + o, spike.getCenterZ() + n), blockState);
                        }
                    }
                }
            }
        ...
        if (Math.abs(((chunk.getPos().x * 16) + 8) - spike.getCenterX()) <= 8 && Math.abs(((chunk.getPos().z * 16) + 8) - spike.getCenterZ()) <= 8) {
            ...
            StructureUtil.setBlockInChunk(chunk, new BlockPos(spike.getCenterX(), spike.getHeight(), spike.getCenterZ()), Blocks.BEDROCK.getDefaultState());
        }
    }
}
